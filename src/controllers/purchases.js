import Purchase from '../ORM/factory/Purchase.js'
...
export const getAll = async (req, res, next) => {
  ...
  try {
    const { rows } = await Purchase.getAll(options)
...
export const getItem = async (req, res, next) => {
  try {
    const item = await Purchase.getItem(req.params.id)
...
export const createItem = async (req, res, next) => {
  try {
    const item = await Purchase.createItem(req.body)
...
export const updateItem = async (req, res, next) => {
  try {
    const item = await Purchase.updateItem(req.body)
    return res.status(201).json(item)
  } catch (e) {
    const errors = e.errors ? handleSequelizeError(e) : { message: 'Error en la base de datos' }
    return res.status(409).json(errors)
...
export const deleteItem = async (req, res, next) => {
  try {
    const item = await Purchase.deleteItem(req.body.id)
...
export const deleteAll = async (req, res, next) => {
  try {
    const response = await Purchase.deleteAll()
    return res.status(201).json(response)
  } catch (e) {
    const errors = e.errors ? handleSequelizeError(e) : { message: 'Error en la base de datos' }
    return res.status(409).json(errors)
