import DetailMatch from '../ORM/factory/DetailMatch.js'
...
export const getAll = async (req, res, next) => {
  ...
  try {
    const { rows } = await DetailMatch.getAll(options)
...
export const getItem = async (req, res, next) => {
  try {
    const item = await DetailMatch.getItem(req.params.id)
...
export const createItem = async (req, res, next) => {
  try {
    const item = await DetailMatch.createItem(req.body)
...
export const updateItem = async (req, res, next) => {
  try {
    const item = await DetailMatch.updateItem(req.body)
    return res.status(200).json(item)
  } catch (e) {
    const errors = e.errors ? handleSequelizeError(e) : { message: 'Error en la base de datos' }
    return res.status(409).json(errors)
...
export const deleteItem = async (req, res, next) => {
  try {
    const item = await DetailMatch.deleteItem(req.body.id)
...
export const deleteAll = async (req, res, next) => {
  try {
    const response = await DetailMatch.deletAll()
    return res.status(200).json(response)
  } catch (e) {
    const errors = e.errors ? handleSequelizeError(e) : { message: 'Error en la base de datos' }
    return res.status(409).json(errors)
